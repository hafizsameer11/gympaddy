...
{
    // Eager load only valid relations
    $adCampaign->load(['adable', 'user']); // remove 'adable.media' to prevent breaking
    ...
    // Attach polymorphic adable content
        ...
        $response['listing'] = [
            ...
            'media' => collect($adable->media_urls)->map(function ($url, $index) {
                return [
                    ...
                    'media_type' => 'image', // or logic to detect video
                    'mime_type' => null,
                    'file_size' => null,
                    'order' => $index,
                    'url' => $url,
